...
(function () {
  ...
  var tickLoop, parallaxBound = false, parOn = false;
  ...
  function startParallax() {
    ...
    window.addEventListener('pointerleave', function () {
      pointer.x = pointer.y = 0; ndc.x = 10; pointerPosition = null;
    });
  }
  ...
  async function build() {
    ...
    buildAmbient();
    layout();
    window.addEventListener('resize', function () { layout(); startTick(); });
  ...
  requestAnimationFrame(function () { requestAnimationFrame(function () {
    build().catch(function (err) { console.error(err); });
  }); });
