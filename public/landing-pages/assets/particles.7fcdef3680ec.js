...
function createParticleScroll(elements, options = {}) {
  ...
  let contentDirty = false, hasPendingContent = true, uploadedContent = false;
  let wake = () => {};
  ...
  syncCanvasSize();
  function uploadContent() { if (!contentDirty) return; contentDirty = false; uploadedContent = hasPendingContent; introReady = true; gl.bindTexture(gl.TEXTURE_2D, contentTexture); gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, source); gl.generateMipmap(gl.TEXTURE_2D); }
  ...
  let rowUnits = new Int32Array(0), rowUnitsDirty = true;
    ...
    for (let i = 0; i < docRows; i++) {
      ...
      if (reducedMotion || !inWin) { if (p !== target) { rowProgress[i] = target; p = target; } }
        ...
        if (target >= COMMIT) {
          ...
          const next = Math.fround(Math.min(1, Math.max(Math.min(target, p + cap), p + dt / hold)));
          if (next > p) { p = next; rowsAnimating = true; rowProgress[i] = p; }
        } else if (p < target) {
          const next = Math.fround(Math.min(p + cap, target));
          if (next > p) { p = next; rowsAnimating = true; rowProgress[i] = p; }
        } else if (p > target) {
          const next = Math.fround(Math.max(p - dt / (settle * 0.6), target));
          if (next < p) { p = next; rowsAnimating = true; rowProgress[i] = p; }
        }
      }
  ...
  function render(dt) {
    ...
    if (rowsAssembled || !uploadedContent) { gl.disable(gl.BLEND); return; }
  ...
  function frame(now) {
    ...
    render(delta);
    if (!contentDirty && scrollSmooth === scrollTop && !rowsAnimating && (rowsAssembled || !uploadedContent) && introDone && lag === 0) { running = false; return; }
  ...
  return {
    ...
    setUnits(next) { units = next || []; rowUnitsDirty = true; start(); },
    repaint() { requestPaint(); start(); },
    destroy() { destroyed = true; cancelAnimationFrame(raf); playbackObserver.disconnect(); document.removeEventListener('visibilitychange', syncParticlePlayback); (scrollTarget || content).removeEventListener("scroll", onScroll); observer.disconnect(); motionQuery.removeEventListener("change", onMotionChange); gl.deleteTexture(contentTexture); gl.deleteTexture(rowTex); gl.deleteProgram(base.program); gl.deleteProgram(points.program); gl.deleteShader(base.vert); gl.deleteShader(base.frag); gl.deleteShader(points.vert); gl.deleteShader(points.frag); gl.deleteBuffer(quad); gl.deleteVertexArray(quadVao); gl.deleteVertexArray(pointVao); },
  };
}
...
function rasterise(tracked, source, ctx) {
  ...
  const vh = window.innerHeight, vw = window.innerWidth, margin = 600;
  let hasPendingContent = false;
  for (const t of tracked) {
    if (t.assembled) continue;
    const box = t.el.getBoundingClientRect(); if (box.bottom < -margin || box.top > vh + margin) continue;
    if (box.bottom > 0 && box.top < vh && box.right > 0 && box.left < vw && box.width > 0 && box.height > 0) hasPendingContent = true;
