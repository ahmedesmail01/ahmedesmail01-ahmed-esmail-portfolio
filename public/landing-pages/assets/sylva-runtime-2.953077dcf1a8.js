...
new ResizeObserver(() => { needResize = true; frameLoop.invalidate(); }).observe(stage);
...
function frame(now){
  ...
  const wantWell = !calm.matches && (on.over || on.press) ? 1 : 0;
...
window.__seek   = v => { clock = v; drawn = null; frameLoop.invalidate(); };
}
